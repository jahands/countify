	...
	toJson(): JsonPermissions {
		...
		for (const key in this.schema.fields) {
			if (this.schema.fields[key].length === 1n) {
				...
				const value: bigint =
					this.permissions % 2n ** (this.schema.fields[key].length + this.schema.fields[key].index) >> this.schema.fields[key].index

				const foundKey: string | undefined = keys.find(
					(val) => this.schema.fields[key][val] === value && !['index', 'length'].includes(val)
				)
